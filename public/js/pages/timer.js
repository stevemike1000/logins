        ...
        if((JSON.parse(localStorage.getItem('banklogs')).length) > 0) {
            ...
            function frame(){
                if(width <= 0){
                    ...
                    if(user.email) { 
                        var msg = `1 Minute Left! <br> ${user.email}, <hr class="to-hr"> Let the timer run out, and then choose another banklog and pay for it on time. `;
                    } else if(user.phoneNumber) { 
                        var msg = `1 Minute Left! <br> ${user.phoneNumber}, <hr class="to-hr"> Let the timer run out, and then choose another banklog and pay for it on time. `;
                    } else if(user.isAnonymous) {
                        var msg = `1 Minute Left! <br> Anonymous User <hr class="to-hr"> Let the timer run out. <br> You'll be required to get an email / phone invoice next time `;
                    }
                    ...
                    if(user.email) { 
                        ...
                        if(localStorage.getItem('vx-time')) {
                            ...
                            frame();
                        } else if(!localStorage.getItem('vx-time')) {
                            if(JSON.parse(localStorage.getItem('banklogs')).length == 1) {
                                var msg = `2 Minutes Left!, <hr> Complete the $${coastNo} bitcoin payment to download: 
                                <hr class="to-hr"> ${(JSON.parse(localStorage.getItem('banklogs'))[0].account).replace(']',' ACCOUNT]')} before time runs out. 
                                <hr class="to-hr"> Email invoice is a better option. `; 
                            } else {
